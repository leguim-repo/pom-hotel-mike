...
! function(e, t) {
	...
	function stringToRawPath(e) {
		...
		for(t = 0; t < w; t++)
			if(g = r, isNaN(m[t]) ? o = (r = m[t].toUpperCase()) !== m[t] : t--, s = +m[t + 1], i = +m[t + 2], o && (s += x, i += y), t || (h = s, u = i), "M" === r) a && (a.length < 8 ? --p.length : C += a.length), x = h = s, y = u = i, a = [s, i], p.push(a), t += 2, r = "L";
			else if("C" === r) o || (x = y = 0), (a = a || [0, 0]).push(s, i, x + 1 * m[t + 3], y + 1 * m[t + 4], x += 1 * m[t + 5], y += 1 * m[t + 6]), t += 6;
		else if("S" === r) f = x, c = y, "C" !== g && "S" !== g || (f += x - a[a.length - 4], c += y - a[a.length - 3]), o || (x = y = 0), a.push(f, c, s, i, x += 1 * m[t + 3], y += 1 * m[t + 4]), t += 4;
		else if("Q" === r) f = x + 2 / 3 * (s - x), c = y + 2 / 3 * (i - y), o || (x = y = 0), x += 1 * m[t + 3], y += 1 * m[t + 4], a.push(f, c, x + 2 / 3 * (s - x), y + 2 / 3 * (i - y), x, y), t += 4;
		else if("T" === r) f = x - a[a.length - 4], c = y - a[a.length - 3], a.push(x + f, y + c, s + 2 / 3 * (x + 1.5 * f - s), i + 2 / 3 * (y + 1.5 * c - i), x = s, y = i), t += 2;
		else if("H" === r) ib(x, y, x = s, y), t += 1;
		else if("V" === r) ib(x, y, x, y = s + (o ? y - x : 0)), t += 1;
		else if("L" === r || "Z" === r) "Z" === r && (s = h, i = u, a.closed = !0), ("L" === r || .5 < F(x - s) || .5 < F(y - i)) && (ib(x, y, s, i), "L" === r && (t += 2)), x = s, y = i;
		else if("A" === r) {
			if(v = m[t + 4], d = m[t + 5], f = m[t + 6], c = m[t + 7], n = 7, 1 < v.length && (v.length < 3 ? (c = f, f = d, n--) : (c = d, f = v.substr(2), n -= 2), d = v.charAt(1), v = v.charAt(0)), l = arcToSegment(x, y, +m[t + 1], +m[t + 2], +m[t + 3], +v, +d, (o ? x : 0) + 1 * f, (o ? y : 0) + 1 * c), t += n, l)
	...
	var y, i, t, r = "CustomEase",
		...
		n = ((t = CustomEase.prototype).setData = function setData(e, t) {
			t = t || {};
			var n, s, i, r, o, a, h, u, f, c = (e = e || "0,0,1,1").match(x),
				l = 1,
				g = [],
				v = [],
				d = t.precision || 1,
			...
			var n, i, r, o, a, h, u, f, c, l, g = (t = t || {}).width || 100,
				v = t.height || 100,
				d = t.x || 0,
				p = (t.y || 0) + v,
			...
			if(t.invert && (v = -v, p = 0), "string" == typeof e && (e = y.parseEase(e)), e.custom && (e = e.custom), e instanceof CustomEase) n = function rawPathToString(e) {
				...
				for(var a, h, u, f, c, l = e.length; - 1 < --l;)
					for(h = (a = e[l]).length, u = 0; u < h; u += 2) f = a[u], c = a[u + 1], a[u] = f * t + c * s + r, a[u + 1] = f * n + c * i + o;
				return e._dirty = 1, e
